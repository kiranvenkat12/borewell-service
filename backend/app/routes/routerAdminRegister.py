...
from app.services.crudAdminRegister import create_admin, login_admin, get_admin
from app.schemas.schemasAdminRegister import CreateRegistration, LoginAdmin
from app.core.dependency import get_current_user  # Added for authentication dependency
...
@router.get("/admins")
def get_admins_endpoint(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    return get_admin(db)