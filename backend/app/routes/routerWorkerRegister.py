...
from app.services.crudWorkerRegister import create_worker_register, get_all_workers, get_worker_by_email, get_worker_by_id
...
def get_all_workers_endpoint(db:Session=Depends(get_db), ):
    return get_all_workers(db)


@worker_register_router.get("/worker-registers/{email}", response_model=WorkerResponse)
def get_worker_by_email_endpoint(email: str, db: Session = Depends(get_db)):
...
def get_worker_by_id_endpoint(id:int, db:Session=Depends(get_db)):
